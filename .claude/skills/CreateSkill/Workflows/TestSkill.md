...
**Workspace:** `<scratch>/skill-test-[skillname]/iteration-[N]/` — `<scratch>` is the session scratchpad if the harness provides one, otherwise `${TMPDIR:-/tmp}`. Keep it out of the repo.
...
Launch all with-skill + baseline pairs in one turn.
