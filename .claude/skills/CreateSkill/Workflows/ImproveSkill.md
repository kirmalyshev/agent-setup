...
2. **Test results** (if from TestSkill): Read outputs from the TestSkill workspace, `<scratch>/skill-test-[name]/`
...
- **Version:** hand-bump the skill's `version:` per the `## Versioning` rubric in CreateSkill's SKILL.md.
