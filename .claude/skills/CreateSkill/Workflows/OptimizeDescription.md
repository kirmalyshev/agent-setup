...
First, collect all skill names and descriptions — personal and project skills both compete for the same prompt:

```bash
rg -L --no-ignore --no-filename --glob 'SKILL.md' '^(name|description):' ~/.claude/skills .claude/skills 2>/dev/null | head -200
```

`-L` matters: skills installed as symlinks are skipped without it. Plugin skills compete too — add any from the session's skill list that share vocabulary with the skill under test.
...
- Stay under 1024 characters (hard limit)
