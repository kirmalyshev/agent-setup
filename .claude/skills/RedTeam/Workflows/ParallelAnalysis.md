...
**Decompose first.** Run the Deconstruct steps in `Integration.md` to separate established components from asserted ones and surface the gap between the stated argument and the actual one. Then break the argument into atomic, independently-attackable claims — each self-contained, specific, and challengeable by a competent critic.
...
**Before the counter-argument,** run the Challenge step in `Integration.md` to classify every constraint as HARD (physics/reality — cannot attack), SOFT (policy/choice — can be challenged), or ASSUMPTION (unvalidated — prime target). The most devastating critiques target "constraints" treated as HARD that are actually SOFT.
...
- `Integration.md` for Deconstruct (decomposition) and Challenge (constraint classification)

**Pairs well with:**
- `SystemsThinking` when the weakness looks structural rather than local
- `RootCauseAnalysis` when the argument is about something that already failed
